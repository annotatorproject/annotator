...
*/
long Popup::frameNr;
void Popup::setCurrFrame(long FrameNr) { frameNr = FrameNr; }
...
void Popup::on_btnPopClose_clicked() {
  ...
  for (int i = 0; i < widgetList.count(); i++) widgetList[i]->setText("");
...
void Popup::on_btnPopSave_clicked() {
  ...
  AnnotatorLib::Commands::NewAnnotation *nA =
      new AnnotatorLib::Commands::NewAnnotation(
          nO->getObject(), frame, x, y, width, height, this->session, false);
  ...
  // save item in RAM
  putInRAM(ui->lE_UL->text(), ui->lE_LR->text(), ItemID.toInt(),
           ItemColor.name());
...
void Popup::on_btnPopAddAtr_clicked() {
  ...
  connect(newWidget, SIGNAL(setNewWidget(const QString, const QString)), this,
          SLOT(setNewWidget(const QString, const QString)));
  newWidget->exec();
}
 ...
 *
 */
void Popup::setData(QPointF UL, QPointF LR, QString id, QString Name,
                    QColor borderColor) {
  QPointF ptMin;
  QPointF ptMax;
  ...
  ui->lE_UL->setText("UL:(" + QString::number(ptMin.x()) + ", " +
                     QString::number(ptMin.y()) + ")");
  ui->lE_LR->setText("LR:(" + QString::number(ptMax.x()) + ", " +
                     QString::number(ptMax.y()) + ")");
}
...
QString Popup::removeFileName(QString Projectpath) {
  ...
  Projectpath = Projectpath.left(Projectpath.length() -
                                 (list.value(list.length() - 1).length() + 1));
  return Projectpath;
}
...
void Popup::showPopUp() {
  ...
  FilePath = Projectpath +
             ("/" + QString("%1").arg(frameNr, 8, 10, QChar('0')) + ".xml");
...
bool Popup::isRAMEmpty() {
  if (StartObjetcsList.isEmpty()) return true;
 ...
 *
*/
void Popup::removeRAM() { StartObjetcsList.clear(); }
