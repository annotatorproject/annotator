...
void StatisticsDialog::reload() {
  ...
  ui->tableWidget->setItem(
      2, 1, new QTableWidgetItem(
                QString::number(project->getSession()->getObjects().size())));
  ui->tableWidget->setItem(
      3, 1, new QTableWidgetItem(QString::number(
                project->getSession()->getAnnotations().size())));

  double duration_sec = project->getDuration();
  ui->tableWidget->setItem(1, 2,
                           new QTableWidgetItem(QString::number(duration_sec)));
  ui->tableWidget->setItem(
      2, 2,
      new QTableWidgetItem(QString::number(
          duration_sec / (double)project->getSession()->getObjects().size())));
  ui->tableWidget->setItem(
      3, 2, new QTableWidgetItem(QString::number(
                duration_sec /
                (double)project->getSession()->getAnnotations().size())));
