...
AnnotationGraphicsItem::AnnotationGraphicsItem(AnnotatorLib::Annotation *annotation):
     QGraphicsItem::QGraphicsItem(nullptr)
{
    ...
    setPos(annotation->getX() - annotation->getHRadius(), annotation->getY() - annotation->getVRadius());
    initCorners();
    initIdText();

    rectX = 0;
    rectY = 0;
    width = annotation->getHRadius()*2;
    height = annotation->getVRadius()*2;
    cornerXPos = 0;
    cornerYPos = 0;
    cornerWidth = width,
    cornerHeight = height,
...
void AnnotationGraphicsItem::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    QGraphicsItem::mouseDoubleClickEvent(event);
    if(this->player != nullptr && this->annotation != nullptr && this->annotation->getObject() != nullptr)
    {
...
*/
bool AnnotationGraphicsItem::sceneEventFilter(QGraphicsItem * watched, QEvent * event )
{
        ...
        case QEvent::GraphicsSceneMouseRelease:
            {
                ...
                changeAnnotationSize((int)this->x(), (int)this->y(), (int)this->width, (int)this->height);
            }
            break;
...
void AnnotationGraphicsItem::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
...
{
    annotation->setPositionWH(annotation->getX(), annotation->getY(), x, y);
}
...
void AnnotationGraphicsItem::changeAnnotationPosition(int x, int y)
{
    ...
    if(annotation->isInterpolated()){
        ...
        AnnotatorLib::Commands::NewAnnotation * nA =
                new AnnotatorLib::Commands::NewAnnotation(annotation->getObject(), annotation->getFrame(),
                    x + annotation->getHRadius(), y + annotation->getVRadius(),
                    annotation->getHRadius(), annotation->getVRadius(),
                    annotation->getNext(), annotation->getPrevious(), player->getSession(), true);
        player->getSession()->execute(nA);
    }else{
        AnnotatorLib::Commands::UpdateAnnotation * uA = new AnnotatorLib::Commands::UpdateAnnotation(annotation,
                x + annotation->getHRadius(), y + annotation->getVRadius(), annotation->getHRadius(), annotation->getVRadius());
