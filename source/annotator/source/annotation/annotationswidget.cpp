...
void AnnotationsWidget::setSession(
...
void AnnotationsWidget::resizeEvent(QResizeEvent *event) {
  for (int i = 0; i < ui->treeWidget->columnCount() - 1; ++i)
    ui->treeWidget->setColumnWidth(
        i, ui->treeWidget->width() / ui->treeWidget->columnCount());
...
void AnnotationsWidget::on_refreshSession() { refreshHoleSession(); }

void AnnotationsWidget::on_annotationAdded(
    shared_ptr<AnnotatorLib::Annotation> a) {
  int idx = objectIndexMap[a->getObject()->getId()];
  // update annonation counter
  ui->treeWidget->topLevelItem(idx)->setText(
      0, QString::fromStdString(a->getObject()->getName()) + " (" +
...
void AnnotationsWidget::on_annotationRemoved(
    shared_ptr<AnnotatorLib::Annotation> a) {
  refreshHoleSession();  // TODO: reload object only
}

void AnnotationsWidget::addAnnotation(
    shared_ptr<AnnotatorLib::Annotation> annotation, QTreeWidgetItem *item) {
...
void AnnotationsWidget::on_objectRemoved(
...
void AnnotationsWidget::on_objectSelected(
    shared_ptr<AnnotatorLib::Object> object) {
  for (auto item : ui->treeWidget->selectedItems()) item->setSelected(false);
  if (object) {
    int idx = objectIndexMap[object->getId()];
...
void AnnotationsWidget::on_treeWidget_currentItemChanged(
  ...
  SelectionController::instance()->setSelectedObject(
      session->getAnnotation(id)->getObject());
