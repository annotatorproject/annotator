...
PluginLoader::PluginLoader(QObject *parent) : QObject(parent) { loadPlugins(); }
...
void PluginLoader::setCurrent(QString name) {
  ...
  if (plugin) this->current = plugin;
}

Plugin *PluginLoader::getCurrent() { return current; }
...
void PluginLoader::loadPlugins(QString dir) {
  ...
  for (QString fileName : pluginsDir.entryList(QDir::Files)) {
    ...
    if (plugin) {
      // loads Plugins
      Plugin *plugin_ = qobject_cast<Plugin *>(plugin);
      if (plugin_) {
        addPlugin(plugin_);
      } else {
        delete plugin_;
...
QList<Plugin *> PluginLoader::getPlugins() { return this->plugins; }
